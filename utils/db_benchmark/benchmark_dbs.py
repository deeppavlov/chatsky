...
def delay_config(conf: BenchmarkConfig, delay):
    old_conf = json.loads(conf.json())
    old_conf.pop("delay", None)
    new_conf = BenchmarkConfig(**old_conf, delay=delay)
    return new_conf


delayed_configs = {
    key1 + key2: delay_config(configs[key1], delays[key2])
    for key1 in configs
    for key2 in delays
}
...
for db_name, db_uri in dbs.items():
    benchmark_all(
        benchmark_dir / f"delay-{db_name}.json",
        "Delay comparison",
        description="Compare different delays between db operations",
        db_uri=db_uri,
        benchmark_configs=delayed_configs,
    )
