...
def repeat(priority: Optional[float] = None, *args, **kwargs) -> Callable:
    ...
    :param priority:
        priority of transition. Uses `Actor.label_priority` if priority not defined.
    :type priority: Optional[float]
    """
...
def _get_label_by_index_shifting(
    ...
    **kwargs,
    ...
    :param priority:
        priority of transition. Uses `Actor.label_priority` if priority not defined.
    :type priority: Optional[float]
    :param increment_flag:
        if it is True, label index is incremented by 1,otherwise it is decreased by 1
        Defaults to True
    :type increment_flag: bool
    :param cyclicality_flag:
        if it is True, the iteration over the label list is going cyclically
        (e.g the element with index=len(labels) has index=0)
        Defaults to True
    :type cyclicality_flag: bool
