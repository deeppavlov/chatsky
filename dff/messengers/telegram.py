...
from telegram import InputMediaAnimation, InputMediaAudio, InputMediaDocument, InputMediaPhoto, InputMediaVideo, Update, Sticker as TelegramSticker, Message as TelegramMessage
...
from dff.script.core.message import Animation, Audio, Contact, Document, Image, Invoice, Location, Message, Poll, PollOption, Sticker, Story, Venue, Video


# TODO: do we need Game, VideoNote, Dice classes?
def extract_message_from_telegram(update: TelegramMessage) -> Message:  # pragma: no cover
    ...
    if update.venue is not None:
        message.attachments += [Venue(latitude=update.venue.location.latitude, longitude=update.venue.location.longitude, title=update.venue.title)]
    ...
    if update.story is not None:
        message.attachments += [Story()]
    if update.sticker is not None:
        message.attachments += [Sticker(source=HttpUrl(update.sticker.file_id))]
...
async def cast_message_to_telegram_and_send(update: TelegramMessage, message: Message) -> None:  # pragma: no cover
    if message.attachments is not None:
        ...
        for attachment in message.attachments:
            ...
            if isinstance(attachment, Venue):
                await update.reply_venue(attachment.latitude, attachment.longitude, attachment.title)
            ...
            if isinstance(attachment, Story):
                pass
            if isinstance(attachment, Sticker):
                attachment_bytes = attachment.get_bytes()
                if attachment_bytes is not None:
                    await update.reply_sticker(attachment_bytes)
