...
async def simple_test(db: DBContextStorage, testing_context: Context, context_id: str):
    # Operation WRITE
    await db.set(context_id, testing_context)

    # Operation LENGTH
    assert await db.length() == 1

    # Operation CONTAINS
    assert await db.contains(context_id)

    # Operation READ
    assert await db.get_default(context_id) is not None

    # Operation DELETE
    await db.delete(context_id)

    # Operation CLEAR
    await db.clear()


async def basic_test(db: DBContextStorage, testing_context: Context, context_id: str):
    assert await db.length() == 0
    assert testing_context.storage_key is None

    # Test write operations
    await db.set(context_id, Context())
    await db.contains(context_id)
    await db.length() == 1
    ...
    await db.set(context_id, testing_context)  # overwriting a key
    assert await db.length() == 1
    assert await db.keys() == {context_id}

    # Test read operations
    new_ctx = await db.get_default(context_id)
    ...
    await db.delete(context_id)
    assert not await db.contains(context_id)

    # Test `get` and `get_default` methods
    with pytest.raises(KeyError):
        await db.get(context_id)
    assert await db.get_default(context_id) is None
...
async def partial_storage_test(db: DBContextStorage, testing_context: Context, context_id: str):
    # Write and read initial context
    await db.set(context_id, testing_context)
    read_context = await db.get_default(context_id)
    ...
    await db.set(context_id, read_context)
    read_context = await db.get_default(context_id)
    assert write_context == read_context.model_dump()


async def midair_subscript_change_test(db: DBContextStorage, testing_context: Context, context_id: str):
...
async def large_misc_test(db: DBContextStorage, testing_context: Context, context_id: str):
    ...
    new_context = await db.get_default(context_id)
    assert len(new_context.misc) == len(testing_context.misc)
    for i in range(100000):
        assert new_context.misc[f"key_{i}"] == f"data number #{i}"


async def many_ctx_test(db: DBContextStorage, _: Context, context_id: str):
    # Set all appended request to be written
    db.context_schema.append_single_log = False
    ...
    assert await db.length() == 100

    # Check that both misc and requests are read as expected
    for i in range(1, 101):
        read_ctx = await db.get_default(f"{context_id}_{i}")
...
async def keys_test(db: DBContextStorage, testing_context: Context, context_id: str):
    # Fill database with contexts
    for i in range(1, 11):
        await db.set(f"{context_id}_{i}", Context())
        sleep(0.001)
...
async def single_log_test(db: DBContextStorage, testing_context: Context, context_id: str):
...
async def run_all_functions(db: Union[DBContextStorage, Dict], testing_context: Context, context_id: str):
    ...
    for test in _TEST_FUNCTIONS:
        ...
        await db.clear(prune_history=True)
        await test(db, Context.cast(frozen_ctx), context_id)
