...
These objects are dictionaries of particular structure:
    ...
    `services` (required) - a ServiceGroupBuilder object, basically a list of
                ServiceBuilder or ServiceGroupBuilder objects, see example №4
    `wrappers` - a list of pipeline wrappers, see example №7
    `timeout` - pipeline timeout, see example №5
    `optimization_warnings` - whether pipeline asynchronous structure should be
                checked during initialization, see example №5
...
ServiceBuilder object can be defined either with callable (see example №2)
            or with dict of following structure / object with following constructor arguments:
    ...
    `name` - custom defined name for the service
                (keep in mind that names in one ServiceGroup should be unique), see example №4
...
def postprocess(ctx: Context, actor: Actor):
    ...
    actor_presence = "" if actor.script[fallback_flow][fallback_node].response == ctx.last_response else " not"
    logger.info(f"actor is{actor_presence} in fallback node")
