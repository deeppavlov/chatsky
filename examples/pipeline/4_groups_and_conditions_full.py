...
Service (and service group) object fields are mostly the same as constructor parameters, however there are some differences:
    ...
    `asynchronous` - combination af `..._async_flag` fields, requested value overrides calculated (if not None), see example №5
...
By default all the services start unconditionally. There are number of built-in condition functions as well as possibility to create custom ones.
Custom condition functions can rely on data in `ctx.misc` as well as on any external data source.
Built-in condition functions check other service states. All of the services store their execution status in context, this status can be one of the following:
...
There are following built-in condition functions:
    ...
    `service_successful_condition(path)` - function that checks, whether service with given `path` executed successfully (is `FINISHED`)
    ...
    `aggregate_condition(aggregator, *functions)` - function that aggregated results of numerous `functions` (condition functions) using special `aggregator` function
    `all_condition(*functions)` - function that returns True only if all of the given `functions` (condition functions) return True
...
pipeline_dict = {
    "components": [
        ...
        ],  # Despite this is the only unnamed service group in the root service group, it will be named `service_group_0`
        actor,
        ServiceGroup(
            name="named_group",
            components=[
