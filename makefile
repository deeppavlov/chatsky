...
VENV_PATH = venv
VERSIONING_FILES =  setup.py makefile docs/source/conf.py df_script_viewer/__init__.py
...
lint: venv
	$(VENV_PATH)/bin/flake8 --max-line-length 120 df_script_viewer/
	@set -e && $(VENV_PATH)/bin/black --exclude="setup\.py" --line-length=120 --check . || ( \
		echo "================================"; \
		echo "Bad formatting? Run: make format"; \
		echo "================================"; \
		false)
	$(VENV_PATH)/bin/mypy df_script_viewer/

.PHONY: lint

test: venv
	$(VENV_PATH)/bin/pytest --cov-report html --cov-report term --cov=df_script_viewer tests/
.PHONY: test
...
doc: venv
	$(VENV_PATH)/bin/sphinx-apidoc -e -f -o docs/source/apiref df_script_viewer
