...
test: venv
	source <(cat .env_file | sed 's/=/=/' | sed 's/^/export /') && pytest -m "not slow" --cov-fail-under=$(TEST_COVERAGE_THRESHOLD) --cov-report html --cov-report term --cov=dff --allow-skip=$(TEST_ALLOW_SKIP) tests/
.PHONY: test
