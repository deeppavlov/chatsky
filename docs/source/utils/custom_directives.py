    ...
    def run(self):
        ...
        try:
            ...
            if "figure" in self.options:
                ...
                thumbnail_rst = re.sub(r"..\sfigure::\s.*\.png", ".. figure:: /{}".format(save_figname), thumbnail_rst)
...
class CustomGalleryItemDirective(Directive):
    ...
    optional_arguments = 0
    final_argument_whitespace = True
    option_spec = {"tooltip": directives.unchanged, "figure": directives.unchanged, "description": directives.unchanged}
    ...
    def run(self):
        ...
        thumbnail_rst = GALLERY_TEMPLATE.format(tooltip=tooltip, thumbnail=thumbnail, description=description)
        thumbnail = StringList(thumbnail_rst.split("\n"))
        thumb = nodes.paragraph()
        self.state.nested_parse(thumbnail, self.content_offset, thumb)
        return [thumb]
